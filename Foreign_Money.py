...
#                print(ext, word, split[word])
    ...
    # Sets the amount in the entry to CAD. Must start from a foreign currency
    # Exchange rate is to 1 CAD
    def set_amount_to_cad(self, exchange_rate):
        self.amount = str(round(float(self.amount) / float(exchange_rate), 2))
...
## Function that writes entries back into the evernote format
def write_entries_evernote_format(entries, evernote_format_file):
    
    f_foreign_money = open(evernote_format_file, 'w+')
    
    day = entries[0].get_day()
    f_foreign_money.write(day + ':\n')
    
    for entry in entries:
        if entry.get_day() == day:
            f_foreign_money.write(entry.get_amount() + ' ' + entry.get_description() + '\n')
        else:
            day = entry.get_day()
            f_foreign_money.write('\n' + day + ':\n')
            f_foreign_money.write(entry.get_amount() + ' ' + entry.get_description() + '\n')
    
    f_foreign_money.close()
    
    print('Wrote ' + str(len(entries)) + ' entries into ' + evernote_format_file)
    
    


## Function that creates a csv output for google sheets and money pro, as well as
## writing the entires in the regular evernote format
def write_quick():
    # Load the accounts dictionary and the categories csv
    accounts_file = 'acc_dic.csv'
    categories_file = 'categories.csv'
    ...
    outMoney = 'money.csv'
    separator = ','
    evernote_format_file = 'from_foreign.txt'
    
    # Get the entries from the input file
    entries = get_entries(inFile, categories, accounts)
    
    # Ask for the exchange rate as an input (1 CAD = what?)
    exchange_rate = input('What is the exchange rate? ')
    
    # Change all of the entry amounts into canadian dollars
    for entry in entries:
        entry.set_amount_to_cad(exchange_rate)
        
    # Write the evernote format file
    write_entries_evernote_format(entries, evernote_format_file)
...
if __name__ == '__main__':
    ...
    # Create the sheets and money csv, and the evernote format file
    
    # write_quick uses only one exchange that is asked to the user
    write_quick()
    
    # write_with_table uses a table of the exchange rate throughout the days as an input
#    write_with_table()
